...
// Array de exemplo com alguns bolos
let bolos = [
  { id: 1, nome: "Bolo de Chocolate", ingredientes: ["Farinha", "Ovos", "Chocolate"], tempoDePreparo: "1 hora" },
  { id: 2, nome: "Bolo de Laranja", ingredientes: ["Farinha", "Ovos", "Laranja"], tempoDePreparo: "50 minutos" },
  { id: 3, nome: "Bolo de Cenoura", ingredientes: ["Farinha", "Ovos", "Cenoura"], tempoDePreparo: "1 hora e 15 minutos" },
];
...
// Rota para obter todos os bolos (protegida por autenticação)
app.get('/bolos/listar', authenticate, (req, res) => {
  res.json(bolos); // Responde com o array de bolos em formato JSON
});

// Rota para obter um servidor por ID (protegida por autenticação)
app.get('/bolos/listarServidor/:id', authenticate, (req, res) => {
  ...
  const servidor = bolos.find(s => s.id === id); // Encontra o servidor com o ID especificado
  if (servidor) {
...
app.post('/bolos/inserir', authenticate, (req, res) => {
  const novoServidor = req.body; // Obtém o novo servidor do corpo da requisição
  novoServidor.id = bolos.length ? bolos[bolos.length - 1].id + 1 : 1; // Define o ID do novo servidor
  bolos.push(novoServidor); // Adiciona o novo servidor ao array
...
app.put('/bolos/atualizar/:id', authenticate, (req, res) => {
  const id = parseInt(req.params.id); // Converte o parâmetro ID para um número inteiro
  console.log(`Recebida requisição PUT para ID: ${id} com dados: `, req.body); // Loga o ID e os dados recebidos
  const index = bolos.findIndex(s => s.id === id); // Encontra o índice do servidor com o ID especificado
  if (index !== -1) {
    bolos[index] = { ...bolos[index], ...req.body }; // Atualiza o servidor com os novos dados
    res.json(bolos[index]); // Responde com o servidor atualizado
  } else {
    res.status(404).send('Servidor não encontrado'); // Responde com status 404 se o servidor não for encontrado
  }
});
...
app.patch('/bolos/atualizar/:id', authenticate, (req, res) => {
  const id = parseInt(req.params.id); // Converte o parâmetro ID para um número inteiro
...
app.delete('/bolos/deletar/:id', authenticate, (req, res) => {
  const id = parseInt(req.params.id); // Converte o parâmetro ID para um número inteiro
  console.log(`Recebida requisição DELETE para ID: ${id}`); // Loga o ID recebido
  const index = bolos.findIndex(s => s.id === id); // Encontra o índice do servidor com o ID especificado
  if (index !== -1) {
    bolos.splice(index, 1); // Remove o servidor do array
...
// Rota para receber campos da lista de bolos no corpo da requisição
app.post('/sua-rota', (req, res) => {
  ...
  // Por exemplo, você pode adicioná-los à sua lista de bolos
  bolos.push({ id, nome, cargo, departamento });
